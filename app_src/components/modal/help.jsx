    ...
    return (
        <React.Fragment>
            ...
            <div className="app-modal-footer hostBrdTopContrast">
                <b>{config.appTitle}</b> ({locale.helpVersion}: {config.appVersion}), {locale.helpAuthor}: <span className="link" onClick={() => openUrl(config.authorUrl)}>{config.authorName}</span>
            </div>
        </React.Fragment>
    );
});
